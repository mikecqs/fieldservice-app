...
import { adicionarItemModelo, removerItemModelo } from "../actions";

type Item = { id: string; descricao: string; quantidade: number; valor_unitario: number };

export default function ItensEditorModelo({
  templateId,
  items,
  ivaPercent,
  totais,
  ...
  async function onAdicionar(formData: FormData) {
    setErro(null);
    const resultado = await adicionarItemModelo(templateId, formData);
    if (resultado?.erro) setErro(resultado.erro);
  }

  async function onRemover(itemId: string) {
    setErro(null);
    const resultado = await removerItemModelo(templateId, itemId);
    if (resultado?.erro) setErro(resultado.erro);
  }
