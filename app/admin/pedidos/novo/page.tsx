  ...
  return (
    <div className="mx-auto max-w-lg">
      ...
      <div className="rounded-xl border border-slate-200 bg-white p-6">
        <h1 className="mb-4 text-lg font-bold text-slate-900">Novo pedido</h1>
        {(clients ?? []).length === 0 ? (
          ...
          <form action={criarPedido} className="grid grid-cols-1 gap-3 sm:grid-cols-2">
            ...
            <label className="block">
              <span className="mb-1 block text-xs font-medium text-slate-600">Origem (opcional)</span>
              <input name="origem" placeholder="Telefone, email…" className="w-full rounded-md border border-slate-300 px-3 py-2 text-sm" />
            </label>
            <label className="col-span-2 block">
            ...
            <label className="col-span-2 flex items-center gap-2 text-sm text-slate-600">
              <input type="checkbox" name="info_falta" className="rounded border-slate-300" />
              Falta informação do cliente para avançar
            </label>
            <div className="col-span-2 mt-2 flex justify-end gap-2">
              <Link href="/admin/pedidos" className="rounded-md border border-slate-300 px-3.5 py-2 text-sm text-slate-700">
                Cancelar
              </Link>
