...
export default async function HistoricoFinanceiroPage() {
  ...
  return (
    <div>
      <div className="mb-5">
        <h1 className="text-xl font-bold text-slate-900">Histórico financeiro</h1>
        <p className="mt-0.5 text-sm text-slate-500">Validações, correções e faturações mais recentes.</p>
      </div>

      <div className="space-y-2">
        {(eventos ?? []).map((e: any, i: number) => (
          <div key={i} className="rounded-md border border-slate-200 bg-white p-3.5 text-sm">
        ...
        {(eventos ?? []).length === 0 && (
          <p className="py-10 text-center text-sm text-slate-400">Ainda sem histórico financeiro.</p>
        )}
      </div>
    </div>
  );
